...
object FullResImageViewUtil2 {
    ...
    fun getScaleAndCenter(
        ...
        // TODO(b/440567345): Do not use Point where x and y are immutable. Use data class instead.
        // Preserve precision by not converting scale to int but the result
        val adjustedViewSize =
            Point((viewSize.x * systemScale).toInt(), (viewSize.y * systemScale).toInt())
        // defaultRawWallpaperRect represents a brand new wallpaper preview with no crop hints.
        val defaultRawWallpaperRect =
            WallpaperCropUtils.calculateVisibleRect(rawWallpaperSize, adjustedViewSize)
